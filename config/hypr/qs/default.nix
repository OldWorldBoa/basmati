...
{
  home.activation = {
  rmStyle = lib.hm.dag.entryBefore ["writeBoundary"] ''
    run rm -rf ~/.config/qs/*
    '';

  setStyleAction = lib.hm.dag.entryAfter ["writeBoundary"] ''
    run cp ~/basmati/config/hypr/qs/src/* ~/.config/qs/
    run cat > ~/.config/qs/Theme.qml << 'EOF'
import QtQuick
import Quickshell
pragma Singleton
